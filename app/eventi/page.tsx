    ...
    const fetchEventi = async () => {
      try {
        ...
        if (res.ok) {
          ...
          const now = new Date();
          const oggi = new Date(now.getFullYear(), now.getMonth(), now.getDate(), 23, 59, 59);
          const futuri = (Array.isArray(data) ? data : []).filter((ev: Evento) => {
            ...
            const fineConFineGiornata = new Date(fine.getFullYear(), fine.getMonth(), fine.getDate(), 23, 59, 59);
            return oggi <= fineConFineGiornata;
          }).sort((a, b) => {
            const inizioA = new Date(a.data_evento).getTime();
            const inizioB = new Date(b.data_evento).getTime();
      ...
      <section style={styles.hero}>
        ...
        <div style={styles.heroContent}>
          ...
          <motion.p initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.9, delay: 0.15 }} style={styles.heroText}>
            Scopri gli appuntamenti in arrivo: rievocazioni, escursioni culturali, musica e iniziative ideate per valorizzare la nostra comunità.
          </motion.p>
        </div>
      </section>

      {/* EVENTI GRID / EMPTY STATE */}
      <section style={styles.section}>
        {loading ? (
          <div style={styles.emptyState}>
            <div style={styles.spinner} />
            <p style={styles.emptyText}>Caricamento eventi...</p>
          </div>
        ) : eventi.length === 0 ? (
          <div style={styles.emptyState}>
            <span style={styles.emptyIcon}>📅</span>
            <p style={styles.emptyText}>Nessun evento in programma al momento.</p>
            <p style={styles.emptySubtext}>Stiamo pianificando i prossimi incontri. Torna a trovarci a breve!</p>
          </div>
        ) : (
            ...
            {eventi.map((ev, index) => {
              ...
              return (
                <motion.article
                  ...
                  whileHover={{ y: -6, boxShadow: "0 22px 50px rgba(45, 74, 62, 0.1)" }}
                >
